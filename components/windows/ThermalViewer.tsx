  ...
  const handleFileUpload = useCallback(async (files: FileList) => {
    for (let i = 0; i < files.length; i++) {
      ...
      try {
        ...
        if (thermalImageInfo && thermalImageInfo.url && result.thermalData) {
          const newImageId = generateId();
          const newThermalImage: ThermalImage = {
            id: newImageId,
            name: file.name,
            thermalData: result.thermalData,
            realImage: realImageInfo ? realImageInfo.url : null,
            preRenderedThermalUrl: thermalImageInfo.url,
          };

          addImage(newThermalImage);
          setActiveImage(newThermalImage.id);
        } else {
          console.error('Missing thermal data or URL in server response for file:', file.name, result);
          throw new Error('Missing thermal data or URL in server response.');
        }
